...
import { BarChart3, Settings, Cpu, Lock, LogOut } from "lucide-react"
...
export function SuperAdminLayout({ children }: SuperAdminLayoutProps) {
  ...
  const navItems = [
    ...
    },
    {
      ...
      href: "/super-admin/biometric-devices",
      icon: Cpu,
    },
    {
      label: "Payroll Lock",
      href: "/super-admin/payroll-lock",
      icon: Lock,
    },
  ]
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-sidebar-accent/50 text-sidebar-foreground transition-colors group"
              >
